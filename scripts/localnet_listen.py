#!/usr/bin/env python3
"""LocalNet listen: set_keeper + request_work + mock inner-call run.
...
REQUEST_WORK = Method.from_signature("request_work()uint64")
MK_RUN = Method.from_signature("run(uint64)void")
...
def main() -> None:
    ...
    print(f"target MinimalTarget appId={app_id} creator={creator}", file=sys.stderr)
    ...
    # Fresh recreate apps start with keeper_app=0. If this app already has a
    # keeper, refuse — run localnet_recreate.py first for a clean create.
    before = decode_global(live)
    existing_keeper = int(before.get("keeper_app") or 0)
    if existing_keeper != 0:
        sys.exit(
            f"target appId={app_id} already has keeper_app={existing_keeper}; "
            "run scripts/localnet_recreate.py first"
        )
    ...
    print(
        f"created MockKeeper appId={mock_id} round={mock_conf.get('confirmed-round')}",
        file=sys.stderr,
    )
    ...
    target_addr = get_application_address(app_id)
    calls: list[dict] = []
    ...
    rec = call_abi(algod, addr, signer, app_id, REQUEST_WORK)
    rec["hook"] = "request_work"
    rec["via"] = "creator"
    calls.append(rec)
    print(f"request_work txid={rec['txid']} round={rec['round']}", file=sys.stderr)
    ...
    rec["hook"] = "run"
    rec["via"] = "mock_keeper.run"
    calls.append(rec)
    print(
        f"run via mock txid={rec['txid']} round={rec['round']} inners={rec['innerCount']}",
        file=sys.stderr,
    )
    ...
    payload = {
        ...
        "global": {
            ...
            "work_done": after.get("work_done", 0),
            "last_run_round": after.get("last_run_round", 0),
            "pending": after.get("pending", 0),
        },
        "notes": (
            ...
            "Did not poke upkeep 81 or 87. set_keeper + request_work + "
            "mock_keeper.run() inner-call of run()uint64."
        ),
    }
        ...
        json.dumps(
            {
                ...
                "appId": app_id,
                "mockKeeperAppId": mock_id,
                "work_done": payload["global"]["work_done"],
                "last_run_round": payload["global"]["last_run_round"],
