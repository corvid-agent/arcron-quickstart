      ...
      if (lr.ok) {
        ...
        if (listen && listen.network === "localnet" && Number(listen.appId) === Number(ln.appId)) {
          ...
          line +=
            ...
            " · work_done " + (g.work_done ?? "—") +
            " · last_run " + (g.last_run_round ?? "—") +
...
async function main() {
  flaps(document.getElementById("app"), "0", 10);
  flaps(document.getElementById("upkeep"), "0", 8);
  ...
  const upkeepId = Number(cfg.upkeepId) || 0;
  const keeper = Number(cfg.keeperAppId) || KEEPER;
  const subhead = document.getElementById("subhead");
  flaps(document.getElementById("keeper"), String(keeper), 10);
  flaps(document.getElementById("app"), String(appId), 10);
  flaps(document.getElementById("upkeep"), String(upkeepId), 8);
  ...
  try {
    ...
    if (txid) {
      paint("LIVE", "live", "QUICKSTART — app " + appId);
    } else {
      paint("REGISTERED", "grounded", "QUICKSTART — waiting for execute");
    }
  } catch (e) {
    paint("GROUNDED", "grounded", "QUICKSTART — GROUNDED");
    document.getElementById("err").hidden = false;
